function [block,TypeText] = vfm_plot_V1_1(vfm,lims,type,lat,fileTime1,noplot,File_select)
% 修改版（官方给的有问题）
...
%     correctCBLabel(cb); %Correct the label for if 4 color colorbar
    set(cb,'Limits',[0,len],'Position',[0.85 0.178 0.013 0.75]);%Set size & loc of colorbar
    set(cb,'FontName','Times New Roman','fontsize',fontsize-2,'FontWeight','bold');
    set(cb,'ytick',[0.5:1:0.5+len-1],'yticklabel',TypeText.ByteTxt(1:end));
    % Title and legend
   FigureName=strcat('VFM',': ',fileTime1);
    title(FigureName,'FontName','Times New Roman','Fontsize',fontsize,'FontWeight','bold');
     
     %legend(File_select.filePath{2});
    % 调整X轴
    set(gca,'xtick',[15:round(size(x,1)/8):size(x,1),size(x,1)-15]); 
    line([47.13 47.13],[1 10.45]);
...
%yticks = [30 25 20 18 16 14 12 10 8 7 6 5 4 3 2 1 0.5 0 -0.5]';
global Z
%yticks = [Z(490:5:580)];
