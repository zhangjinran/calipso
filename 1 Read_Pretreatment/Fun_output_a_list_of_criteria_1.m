...
% Parse date limits before scanning yearly file lists.
...
target_date_names = cellstr(datestr(floor(datelim_start):floor(datelim_end), 'yyyy_mm_dd'));
...
for j = 1:length(data_type_original)
    ...
    data_struct.file_time.(type_norm) = {};
...
for j=1:length(data_type_original)
    ...
    if ~isempty(data_struct.folder_path.(type_norm))
        ...
        if isempty(data_struct.file_list.(type_norm))
            tempCells = cell(1, numel(target_date_names));
            for i = 1:numel(target_date_names)
                folderFullPath = fullfile(data_struct.file_year_path.(type_norm), ...
                    target_date_names{i}, '*.hdf');
                tempCells{i} = dir(folderFullPath);
            end
            tempCells = tempCells(~cellfun('isempty', tempCells));
            if isempty(tempCells)
                data_struct.file_list.(type_norm) = dir(fullfile( ...
                    data_struct.file_year_path.(type_norm), '*.hdf'));
            else
...
% Precompute timestamp keys once for product matching.
for j = 1:length(data_type_original)
    type_norm = type_map(data_type_original{j});
    folder_info = data_struct.file_list.(type_norm);
    time_keys = cell(size(folder_info));
    for kk = 1:numel(folder_info)
        dot_idx = strfind(folder_info(kk).name, '.');
        if numel(dot_idx) >= 2
            time_keys{kk} = folder_info(kk).name(dot_idx(1):dot_idx(2));
        else
            time_keys{kk} = '';
        end
    end
    data_struct.file_time.(type_norm) = time_keys;
end
...
for iii = 1:size(folder_temp,1)
    ...
    %disp(timeNum)
    % Apply filename filters before any HDF read.
    if timeNum < datelim_start || timeNum > datelim_end
        statistic.removed = statistic.removed + 1;
        continue;
    end
    if ~(strcmpi(day_night_flag, 'all') || strncmpi(day_night_flag, DayOrNight, 1))
        statistic.removed = statistic.removed + 1;
        continue;
    end
    %筛选合适的年份路径。
    ...
    if (timeNum>=datelim_start && timeNum<=datelim_end) && ...
       ...
       ~isempty(lon_intersect) && ~isempty(lat_intersect)
        ...
        % 匹配同时间戳的其他产品文件
        % Match products through precomputed timestamp keys.
        for j = 1:length(data_type_original)
            type_ori = data_type_original{j};
            type_norm = type_map(type_ori);
            if ~isempty(data_struct.file_list.(type_norm))
                time_keys = data_struct.file_time.(type_norm);
                match_idx = find(strcmp(time_keys, fileTime1), 1);
                if ~isempty(match_idx)
                    folder_info = data_struct.file_list.(type_norm);
                    data_struct.flag.(type_norm) = 1;
                    data_struct.filename.(type_norm) = folder_info(match_idx).name;
                    data_struct.filepath.(type_norm) = folder_info(match_idx).folder;
                end
            end
        end
