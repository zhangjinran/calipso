function Fun_Layer_Plot_Profile(Lat,Lidar_Data_Altitudes,TAB_532,topbin_under80km,basebin_under80km,title_name)
    ...
    for i = 1:size(topbin_temp,1)
        for j = 1:size(topbin_temp,2)
            if ~isnan(topbin_temp(i,j))
                ...
                if size(Lidar_Data_Altitudes,1)==399
                    y2=y2/583*399;
                end
...
set(gca,'FontSize',myfontsize-2,'FontWeight','bold');%
%设置标题
Title="Layer Detection "+title_name;
title(Title,'fontsize',myfontsize,'FontWeight','bold');%标题

h_legend=legend(h_temp,legend_name_temp,'Location','northeast','Orientation','horizontal',...
...
while i<2 %这里设置可手动点击廓线的次数
    ...
    if x_single_profile(i) >= min(X) &&  x_single_profile(i) <= max(X)
        ...
        for j=2:length(yy)
            dd = char(dd,sprintf('%d',round(Lidar_Data_Altitudes(round(yy(j))))));
        end
